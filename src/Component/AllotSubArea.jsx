...
const AllotSubArea = () => {
    ...
    const handleInputChange = (e) => {
        ...
        setSubAreaData({...subareaData,[name]: value});
    };
    const handleSubmit = async (e) => {
        let dataToSend;
        e.preventDefault();
        dataToSend = subareaData;
        try {
            console.log(dataToSend);
            const res = await axios.post("/allotsubarea",dataToSend) ;
            if(res.status==201) {
                toast.success('Sub-Area Alloted Successfully',{
                    position: toast.POSITION.TOP_CENTER,
                    closeButton: false,
                    hideProgressBar: false,
            ...
            <div className="p-5 md:p-10 bg-slate-200 dark:bg-slate-800 min-h-screen flex flex-row gap-6 w-auto md:w-full">
                ...
                <div className="flex flex-col w-full gap-4">
                    ...
                    <div className="px-2 flex flex-col">
                        <h2 className="text-4xl font-bold text-black dark:text-white">Allot Sub-Area</h2>

                        <form className="py-4 flex flex-col gap-4" onSubmit={handleSubmit}>
                            <input type="text" id="securityid" name="securityId" placeholder="SECURITY ID" className="px-4 py-2 w-[40%] rounded-md bg-transparent bg-white dark:bg-slate-700 outline-none text-black dark:text-white border border-blue-600 dark:border-0" value={subareaData.securityId} onChange={handleInputChange}/>

                            <input type="text" id="subarea" name="subarea" placeholder="Sub-Area" className="px-4 py-2 w-[40%] rounded-md bg-transparent bg-white dark:bg-slate-700 outline-none text-black dark:text-white border border-blue-600 dark:border-0" value={subareaData.subarea} onChange={handleInputChange}/>

                            <input type="text" id="area" name="area" placeholder="Area" className="px-4 py-2 w-[40%] rounded-md bg-transparent bg-white dark:bg-slate-700 outline-none text-black dark:text-white border border-blue-600 dark:border-0" value={subareaData.area} onChange={handleInputChange}/>

                            <input type="text" id="city" name="city" placeholder="City" className="px-4 py-2 w-[40%] rounded-md bg-transparent bg-white dark:bg-slate-700 outline-none text-black dark:text-white border border-blue-600 dark:border-0" value={subareaData.city} onChange={handleInputChange}/>

                            <input type="text" list="states" id="state" name="state" placeholder="State" className="px-4 py-2 w-[40%] rounded-md bg-transparent bg-white dark:bg-slate-700 outline-none text-black dark:text-white border border-blue-600 dark:border-0" value={subareaData.state} onChange={handleInputChange}/>
                            {STATES.map((state) => (
                            ...
                            <input type="text" list="country" id="country" name="country" placeholder="Country" className="px-4 py-2 w-[40%] rounded-md bg-transparent bg-white dark:bg-slate-700 outline-none text-black dark:text-white border border-blue-600 dark:border-0" value={subareaData.country} onChange={handleInputChange}/>
                            <datalist id="country">
                                <option value="India"/>
                            </datalist>

                            <button type="submit" className="mt-2 px-4 py-2 text-lg w-[20%] bg-blue-600 dark:bg-white rounded-md font-bold text-white dark:text-black">Allot</button>
                        </form>
                    </div>
                </div>
            </div>
