...
const AllotedSubAreaAdminView = () => {
    ...
    const [allotedsubareaData, setAllotedSubAreaData] = useState([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('/allotedsubareabyadmin');
                setAllotedSubAreaData(response.data);
            }
            catch (error) {
            ...
            <div className="p-5 md:p-10 bg-slate-200 dark:bg-slate-800 min-h-screen flex flex-row gap-6 w-auto md:w-full">
                ...
                <div className="flex flex-col w-full gap-4">
                    ...
                    <div className="w-full flex flex-col">
                        <h2 className="py-5 text-3xl font-bold text-black dark:text-white">Alloted Sub-Areas</h2>
                        <table className="hidden md:inline-block w-full">
                            <thead className="w-full flex flex-col">
                                <tr className="bg-blue-600 text-white rounded-md w-[100%]">
                                    <td className="p-3 w-[20%]">Security ID</td>
                                    <td className="p-3 w-[20%]">Sub-Area</td>
                                    <td className="p-3 w-[20%]">Area</td>
                                    <td className="p-3 w-[20%]">City</td>
                                    <td className="p-3 w-[20%]">State</td>
                                    <td className="p-3 w-[20%]">Country</td>
                                </tr>
                            </thead>
                            <tbody className="py-2 flex flex-col gap-2 w-full">
                            {allotedsubareaData && allotedsubareaData.length > 0 ? (
                                allotedsubareaData.map((data, index) => (
                                <tr key={index} className="p-2 text-black dark:text-white bg-white dark:bg-slate-700 shadow-sm shadow-gray-400 dark:shadow-none rounded-md">
                                    <td className="p-3 w-[20%]">{data.securityId}</td>
                                    <td className="p-3 w-[20%]">{data.subarea}</td>
                                    <td className="p-3 w-[20%]">{data.area}</td>
                                    <td className="p-3 w-[20%]">{data.city}</td>
                                    <td className="p-3 w-[20%]">{data.state}</td>
                                    <td className="p-3 w-[20%]">{data.country}</td>
                                </tr>
                            ))
                            ) : (
                                <tr className="p-2 text-black dark:text-white bg-white dark:bg-slate-700 shadow-sm shadow-gray-400 dark:shadow-none w-[100%]">
                                    <td className="p-3 text-2xl font-bold text-black dark:text-white text-center w-[100%]" colSpan={5}>No Record Found</td>
                                </tr>
                            )}
                            </tbody>
                        </table>
                        <table className="block md:hidden w-full">
                            <thead className="w-full">
                                <tr className="bg-blue-600 text-white rounded-md w-full">
                                    ...
                                    <td className="p-3 w-[20%]">City</td>
                                    <td className="p-3 w-[20%]">State</td>
                                    <td className="p-3 w-[20%] rounded-r-md">Country</td>
                                </tr>
                            </thead>
                            <tbody className="py-2 flex flex-col gap-2 w-full">
                            {allotedsubareaData && allotedsubareaData.length > 0 ? (
                            allotedsubareaData.map((data, index) => (
                                <tr key={index} className="p-2 text-black dark:text-white bg-white dark:bg-slate-700 shadow-sm shadow-gray-400 dark:shadow-none w-[100%] rounded-md">
                                    <td className="p-3 w-[20%]">{data.securityId}</td>
                                    <td className="p-3 w-[20%]">{data.subarea}</td>
                                    <td className="p-3 w-[20%]">{data.area}</td>
                                    <td className="p-3 w-[20%]">{data.city}</td>
                                    <td className="p-3 w-[20%] text-wrap">{data.state}</td>
                                    <td className="p-3 w-[20%]">{data.country}</td>
                                </tr>
                            ))
                            ) : (
                                <tr className="p-2 text-black dark:text-white bg-white dark:bg-slate-700 shadow-sm shadow-gray-400 dark:shadow-none">
...
export default AllotedSubAreaAdminView